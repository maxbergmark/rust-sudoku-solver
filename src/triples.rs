...
fn check_triples_rows_or_cols(sudoku: &mut Sudoku, get_row_or_col: &dyn Fn(usize) -> usize, cols_or_rows: &[[usize; 9]; 9]) -> Result<(), SudokuError> {
...
fn check_digit(sudoku: &mut Sudoku, digit: i32, cache: &[usize; 3], base_row_idx: usize, cell_idx: usize, cols_or_rows: &[[usize; 9]; 9]) -> Result<(), SudokuError> {
    let bitmask = 1 << digit;
    let matching_rows: Vec<usize> = cache.iter().enumerate()
        .filter(|(_, &row_bitmask)| row_bitmask & bitmask > 0)
        .map(|(row_idx, _)| row_idx)
        .collect();

    if let [row_idx] = matching_rows.as_slice() {
        check_triple_digits(sudoku, digit, base_row_idx, *row_idx, cell_idx, cols_or_rows)?;
    }
    Ok(())
}


fn check_triple_digits(sudoku: &mut Sudoku, digit: i32, base_row_idx: usize, row_idx: usize, cell_idx: usize, cols_or_rows: &[[usize; 9]; 9]) -> Result<(), SudokuError> {
...
mod tests {
    ...
    #[test]
    fn test_manual() {
        let input = "057000300300801000081703000600030090020070000800000000400607000000000207700000050";
    ...
    #[test]
    fn test_manual_2() {
        let input = "000000036030000052200000000000067820700000400000053000000706210060900000001000000";
        let mut sudoku = Sudoku::from_str(input).unwrap();
